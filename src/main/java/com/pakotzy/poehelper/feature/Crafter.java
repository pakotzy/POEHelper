...
//		Compile modPatterns
		ArrayList<Pattern> modPatterns = new ArrayList<>();
		if (event.hasModsRequirements()) {
			for (String mod : event.getMods())
				modPatterns.add(Pattern.compile(mod, Pattern.MULTILINE | Pattern.CASE_INSENSITIVE));
		}

//		Compile socketPatterns
